...
### Day 28: 2022-04-24
**Today's Progress**: 完成使用分治法找假硬幣這件事，再練習蒙地卡羅法求pi值。

**Thoughts** 在做這些練習前，先用紙筆做好規畫，才能提升效率。單純靠腦力和指令有點困難，一直在用
print out做try and error，不是太漂亮。一些常見的基本動作，像是輸入、格式化輸出、取亂數等，應
該整理出三種語言的各自做法，至少把基本動作掌握好。

### Day 29: 2022-04-26
**Today's Progress**: 使用C語言實做生命遊戲。

**Thoughts** 程式不難寫，但由於書本上的規則解釋不太清楚，又上了維基查。用亂數產生初始條件，以
迴圈任其演化幾次，觀察演化結果，其實蠻有趣的。

### Day 30: 2022-05-19
**Today's Progress**: 天啊，總算撐到第30天了。《保證學會C/C++ 演算法》書上的範例8.2：約瑟夫
環，是個總共41人，1到3報數，報到3就自殺的遊戲。

**Thoughts** 弄了兩個晚上還是沒有全部弄出來，明明故事邏輯不複雜，可見平常工作用到算法的機會太
少，這種問題都解決不了。這幾次的題目做下來，我發現還是需要傳統的紙筆工作術，先把邏輯弄清楚，一些
必要條件、必要處理邏輯先規畫清楚，寫code才比較簡單些。

### Day 31: 2022-09-02
**Today's Progress**: 把之前練習的Bootstrap Modal成果移植到Flask中，讓sprint-analyzer
當某個Issue被點擊時，以模態視窗顯示工作回報內容。練習到html元件點擊後如何把參數帶給js，js又如何
組成供axios回呼flask所使用的url，最後把jinja template產生的html內容放進模態視窗中顯示出來。

**Thoughts** 中斷了四個月，現實是我沒辦法一直保持在單一主題下，所以雖然原始出發點是
用這個活動來練習C語言，但考量到實際情況，必須開放這個活動的適用範圍。
